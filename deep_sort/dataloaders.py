...
class LoadImages:
    ...
    def __next__(self):
        ...
        if self.video_flag[self.count]:
            ...
            im0 = cv2.putText(im0, 'Frame '+str(self.frame), 
                              (30,100),
                        0,
                        2,
                        (0,0,0),
            ...
            while not ret_val: # no frame / end of video -> read next video
                self.count += 1
                self.cap.release()
                if self.count == self.nf:  # last video
                    raise StopIteration
                path = self.files[self.count]
                self.new_video(path) # read next new video
                ret_val, im0 = self.cap.read()

            # skip some frames, jump forward 100  frames 
            # and # of frame < total # of frames
            #if self.frame+10 > self.cap.get(7):
            #    break
            interval = 10
            self.cap.set(cv2.CAP_PROP_POS_FRAMES, self.frame + interval)
            self.frame += interval
